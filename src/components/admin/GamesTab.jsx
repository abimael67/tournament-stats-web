  ...
  const handleSubmitGame = async (e) => {
    ...
    try {
      ...
      const gameData = {
        ...
        score_team_a:
          formData.status === "completed" || formData.status === "in_progress"
            ? parseInt(formData.score_team_a)
            : null,
        score_team_b:
          formData.status === "completed" || formData.status === "in_progress"
            ? parseInt(formData.score_team_b)
            : null,
      ...
      if (isEditing) {
        ...
        const result = await supabase.from("games").insert([gameData]);
        error = result.error;
      }
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Puntos{" "}
                          {
                            teams.find((t) => t.id === formData.team_a_id)
                              ?.team_name
                          }
                        </label>
                        <input
                      ...
                      <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">
                          Puntos{" "}
                          {
                            teams.find((t) => t.id === formData.team_b_id)
                              ?.team_name
                          }
                        </label>
                        <input
