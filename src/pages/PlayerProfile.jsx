...
const PlayerProfile = () => {
  ...
  const fetchPlayerStatistics = async () => {
    try {
      ...
      if (statsData && statsData.length > 0) {
        const aggregated = statsData.reduce((acc, stat) => {
          acc.totalGames += 1;
        ...
        aggregated.avgPoints = aggregated.totalGames > 0 ? (aggregated.totalPoints / aggregated.totalGames).toFixed(1) : 0;
        aggregated.avgRebounds = aggregated.totalGames > 0 ? (aggregated.totalRebounds / aggregated.totalGames).toFixed(1) : 0;
        aggregated.avgAssists = aggregated.totalGames > 0 ? (aggregated.totalAssists / aggregated.totalGames).toFixed(1) : 0;
        aggregated.fieldGoalPercentage = aggregated.totalFieldGoalAttempts > 0 ? ((aggregated.totalFieldGoalMade / aggregated.totalFieldGoalAttempts) * 100).toFixed(1) : 0;
        aggregated.threePointPercentage = aggregated.totalThreePointAttempts > 0 ? ((aggregated.totalThreePointMade / aggregated.totalThreePointAttempts) * 100).toFixed(1) : 0;
        aggregated.freeThrowPercentage = aggregated.totalFreeThrowAttempts > 0 ? ((aggregated.totalFreeThrowMade / aggregated.totalFreeThrowAttempts) * 100).toFixed(1) : 0;
    ...
    return (
      <div className="container mx-auto px-4 py-8">
        <div className="bg-red-50 border border-red-200 rounded-lg p-6 text-center">
          ...
          <Link to="/jugadores" className="mt-4 inline-block text-blue-600 hover:text-blue-800">
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      {/* Navegación */}
      <div className="mb-6">
        <Link to="/jugadores" className="text-blue-600 hover:text-blue-800 flex items-center gap-2">
          ...
          <div className="flex-grow text-center md:text-left">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="bg-blue-50 rounded-lg p-4">
                ...
                <p className="text-sm text-gray-500">{player.team.church_name}</p>
              </div>
              
              
                <div className="bg-green-50 rounded-lg p-4">
                    <h3 className="font-semibold text-green-800 mb-1">Posición</h3>
                  <p className="text-gray-700">{getPositionName(player.position)}</p>
                  <h3 className="font-semibold text-green-800 mb-1">Edad</h3>
                  <p className="text-gray-700">{player.age ? player.age + ' años' : 'No disponible'}</p>
                </div>
      ...
      {aggregatedStats && (
        <div className="bg-white rounded-lg shadow-lg p-6 mb-8">
          <h2 className="text-2xl font-bold text-blue-800 mb-6">Estadísticas de la Temporada</h2>
          
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-6">
            <div className="bg-blue-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-blue-800">{aggregatedStats.totalGames}</div>
              <div className="text-sm text-gray-600">Partidos</div>
            </div>
            
            <div className="bg-green-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-green-800">{aggregatedStats.avgPoints}</div>
              <div className="text-sm text-gray-600">Pts/Partido</div>
            </div>
            
            <div className="bg-purple-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-purple-800">{aggregatedStats.avgRebounds}</div>
              <div className="text-sm text-gray-600">Reb/Partido</div>
            </div>
            
            <div className="bg-yellow-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-yellow-800">{aggregatedStats.avgAssists}</div>
              <div className="text-sm text-gray-600">Ast/Partido</div>
            </div>
            
            <div className="bg-red-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-red-800">{aggregatedStats.fieldGoalPercentage}%</div>
              <div className="text-sm text-gray-600">% Tiros Campo</div>
            </div>
            
            <div className="bg-indigo-50 rounded-lg p-4 text-center">
              <div className="text-2xl font-bold text-indigo-800">{aggregatedStats.threePointPercentage}%</div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-800">{aggregatedStats.totalPoints}</div>
              <div className="text-sm text-gray-600">Puntos Totales</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-800">{aggregatedStats.totalRebounds}</div>
              <div className="text-sm text-gray-600">Rebotes Totales</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-800">{aggregatedStats.totalAssists}</div>
              <div className="text-sm text-gray-600">Asistencias Totales</div>
            </div>
            
            <div className="text-center">
              <div className="text-lg font-semibold text-gray-800">{aggregatedStats.freeThrowPercentage}%</div>
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        {playerStats.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-gray-500">No hay estadísticas registradas para este jugador</p>
          </div>
        ) : (
            ...
            <table className="min-w-full table-auto">
              <thead>
                <tr className="bg-gray-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">TL</th>
                </tr>
              </thead>
                  ...
                  return (
                    <tr key={stat.id} className={index % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                       <a href={`/partido/${stat.game.id}`}>{new Date(stat.game.date).toLocaleDateString('es-ES')}</a>
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
                        {opponent.team_name}
                      </td>
                      <td className="px-4 py-4 whitespace-nowrap text-sm">
                        <span className={`inline-flex px-2 py-1 text-xs font-semibold rounded-full ${
                          result === 'Victoria' ? 'bg-green-100 text-green-800' :
                          result === 'Derrota' ? 'bg-red-100 text-red-800' :
